...
First off, what even is an option? 

Essentially, an option is a financial contract that gives the holder the right, **but not the obligation,** to buy or sell an asset (usually a stock or index) by some expiration date.

Let's recap **calls and puts.**
...
Consequently, we say that a contract is "out-of-the-money" (OTM) if the price of the underlying is 1. **below** the strike price for **call options** or 2. **above** the strike price for **put options**. In our example, if John owned a call option for $80 per share and not a put option, his contract would be OTM.
...
Previously, we said that in theory, our model would output values **closer and closer to the values generated by Black-Scholes**. So, why don't we just implement the B.S. equation and check our error?
